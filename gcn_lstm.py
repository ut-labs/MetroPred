...
class BiLSTM(nn.Module):
    def __init__(self, dim=64, dim1=16, dim2=16):
...
class GCNNet(nn.Module):
    def __init__(self, node_num):
        ...
        self.conv1 = GCNConv(dim, dim, improved=True)
...
class Model(nn.Module):
    def __init__(self, graph_data):
        ...
        # self.register_parameter('lstm', self.lstm)
        # self.register_parameter('gcn', self.gcn)
    ...
    def forward(self, data):
        ...
        assert  network_emb.shape == (81, 16)
        x = data[:, :, :2]
        new_x1 = self.fc(x)
        ids = data[:, :, 2].long()
...
def main():
    ...
    torch_dataset = Data.TensorDataset(all_data[:,:,:3], all_data[:,:, -3:-1])
