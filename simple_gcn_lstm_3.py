...
class BiLSTM(nn.Module):
    def __init__(self, dim=50, dim1=20, dim2=0):
...
class GCNNet(nn.Module):
    def __init__(self, node_num):
        ...
        # self.conv1 = ChebConv(dim, dim, K=32)
        # self.conv2 = ChebConv(dim, dim, K=32)
...
class Model(nn.Module):
    ...
    def forward(self, data):
        ...
        res = self.lstm(new_x1)
        # print(res.shape, new_x2.shape)
        res = torch.cat([res, new_x2], dim=2)
        result = self.none_linear_layer(res)
...
def main():
    ...
    graph = read_graph_csv('./maps/graph_floyd.csv')
